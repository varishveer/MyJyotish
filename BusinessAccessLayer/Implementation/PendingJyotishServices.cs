...
namespace BusinessAccessLayer.Implementation
{
   ...
   We look forward to speaking with you!";
        ...
        {
            var Data = _context.InterviewMeeting.Include(x => x.Jyotish).ThenInclude(x => x.Slots).Where(x => x.Status == true && x.JyotishId == jytotishId ).Where(y=>y.Jyotish.Slots.FirstOrDefault().Incomplete).Select(x => new InterviewMeetingViewModel
            {
                Id = x.Id,
                Link = x.Link,
                Title = x.Title,
